<div class="mi-ubicacion-form-panel {{ $errors->any() ? '' : 'd-none' }}" id="miUbicacionFormPanel">
    <div class="mb-3">
        <h5 class="mb-1">Actualizar ubicación</h5>
        <small class="text-muted">Ajusta tu punto principal o el radio visible para clientes.</small>
    </div>
    ...
    <form class="needs-validation" novalidate method="POST" action="{{ route('mi-perfil-proveedor.ubicacion.store') }}">
        ...
        <div class="row g-3">
            ...
            <input type="hidden" name="latitud" id="latitud" value="{{ old('latitud', $ubicacion?->latitud ?? '-16.5000000') }}">
            <input type="hidden" name="longitud" id="longitud" value="{{ old('longitud', $ubicacion?->longitud ?? '-68.1500000') }}">
